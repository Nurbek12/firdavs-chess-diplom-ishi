                ...
                <v-card-text class="pa-4">
                    <v-form class="w-100" ref="form">
                        <v-row>
                            ...
                            <v-col cols="12">
                                <v-text-field placeholder="Называние" v-model="eItem.title" class="mt-1" hide-details
                                    density="comfortable" color="primary" flat variant="outlined"></v-text-field>
                            </v-col>
                            <v-col cols="12">
                                <v-text-field type="date" placeholder="Дата" v-model="eItem.date" class="mt-1" hide-details
                                    density="comfortable" color="primary" flat variant="outlined"></v-text-field>
                            </v-col>
                            <v-col cols="6">
                                <v-text-field placeholder="Макс. пользователи" v-model="eItem.max_users" class="mt-1" hide-details
                                    density="comfortable" color="primary" flat variant="outlined" type="number"></v-text-field>
                            </v-col>
                            <v-col cols="6">
                                <v-select placeholder="Тип" v-model="eItem.type" :items="[{title:'Турнир',value:'tournament'},{title:'Сезон',value:'season'}]" class="mt-1" hide-details
                                    density="comfortable" color="primary" flat variant="outlined" />
                            </v-col>
                            <v-col cols="12">
                                <v-text-field type="number" placeholder="Награда" v-model="eItem.reward" class="mt-1" hide-details
                                    density="comfortable" color="primary" flat variant="outlined"></v-text-field>
                            </v-col>
                            <v-col cols="12">
                                <v-textarea placeholder="Описания" v-model="eItem.description" class="mt-1" hide-details
                                    density="comfortable" color="primary" flat variant="outlined" />
                            </v-col>
                            <v-col cols="12">
...
const save = async () => {
    ...
    Object.keys(eItem).map((key) => {
        formdata.append(key, eItem[key as keyof typeof eItem])
    })
